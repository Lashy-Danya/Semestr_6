/*****************************************************************************
 Помощь в выборе аквариумной рыбы
******************************************************************************/
...
/* Размещение в резидентной БД информации из утверждений БЗ ЭС */
     ...
     assert_database:-!.
/* Условия-характеристики различных видов аквариумных рыб.*/
     cond(1,"холодноводная").
     cond(2,"тепловодная").
     cond(3,"Pазмер тела до 8 см").
     cond(4,"Pазмер тела до 15 см").
     cond(5,"Pазмер тела до 30 см").
     cond(6,"Tело удлиненной формы").
     cond(7,"Tело ромбовидной формы").
     cond(8,"Вуалевая форма плавников").
     cond(9,"Жировой плавник").
     cond(10,"На хвосте красное пятно").
     cond(11,"На хвосте черное пятно").
     cond(12,"Серебристый окрас тела").
     cond(13,"Ярко-красный окрас тела").
     cond(14,"Зеленый окрас тела").
     cond(15,"Коричневый окрас тела").
     cond(16,"Серый окрас тела").
     cond(17,"Синий окрас тела").
     cond(18,"Пара усов").
     cond(19,"Необходимость в сородичах").
     cond(20,"Миролюбивый характер").
     cond(21,"Агрессивный характер").
/* Данные о типах аквариумных рыб */
     topic("холодноводная").
     topic("тепловодная").
/* Данные о конкретных рыбах */
     rule(1,"Аквариумная рыба","cold water",[1]).
     rule(2,"Аквариумная рыба","warm",[2]).
     rule(3,"холодноводная","Данио жемчужный",[3,6,8,10,12,19,20]).
     rule(4,"холодноводная","Тетра бриллиантовая",[3,7,9,12,19,20]).
     rule(5,"холодноводная","Барбус огненный",[3,6,11,13,19,20]).
     rule(6,"холодноводная","Боция мраморная",[4,6,12,19,20]).
     rule(7,"холодноводная","Брохис",[3,6,14,18,19,20]).
     rule(8,"холодноводная","Торакатум",[4,6,15,18,19,20]).
     rule(9,"тепловодная","Гуппи кобра",[3,6,8,15,19,20]).
     rule(10,"тепловодная","Геллера",[4,6,14,19,20]).
     rule(11,"тепловодная","Акара",[5,7,16,19,21]).
     rule(12,"тепловодная","Цихлазома Северум",[4,7,13,19,21]).
     rule(13,"тепловодная","Макропод",[4,6,17,19,21]).
     rule(14,"тепловодная","Гурами",[4,6,17,19,20]).
     do_consulting:-
           goes(_,First_keyword),
           go([],First_keyword),!.
     do_consulting:-
           not(dummy),
           dlg_Error("Информация об интересующей Вас аквариумной рыбе отсутствует в базе знаний."),
           clear.  
/* Выдача подсказки */
     info:-
           ...
           dlg_Note("База знаний содержит информацию об аквариумных рыбах: ",Breed_type_list_str_repr1),
           assertz(dummy).
...
/* Ввод запроса на естественном (русском) языке */
     goes(Mygoal,First_keyword):-
           Mygoal=dlg_GetStr("Консультация","Введите Ваш запрос (холодноводная или тепловодная) : ","Текст запроса"),
...
/* Начальное правило механизма вывода */
     go(_,Mygoal):-
           ...
           concat("Рекомендуемая аквариумная рыба: ",Mygoal,Temp),
           concat(Temp,".",Result),
