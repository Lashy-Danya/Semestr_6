...
Mx = integrate.quad(lambda x: x * (1 / (2 * np.sqrt(x))), A, B)
Mx = Mx[0]

# нахождение дисперсии
integrate_for_Dx = integrate.quad(lambda x: x**2 * (1 / (2 * np.sqrt(x))), A, B)
...
for item in range(len(size_list)):
    ...
    new_num_np = np.power(num_seq_np, 2) + 2 * num_seq_np + 1
